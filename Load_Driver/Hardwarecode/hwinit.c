...
void hw_init(void)
{
    ...
    PID_init(&Load2_id_pi, 6, 500, 0, 86); // Current PI Init
    PID_init(&Load2_iq_pi, 6, 500, 0, 86);
		// 1us Timer
    ...
    while (Load1_curr.sample_flag == CURR_SAMPLE_GET_OFFSET || Load2_curr.sample_flag == CURR_SAMPLE_GET_OFFSET)
    {
        ;
    }
    // DAC Init
