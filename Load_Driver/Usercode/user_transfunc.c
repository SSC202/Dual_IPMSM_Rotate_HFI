 ...
 * @note    该函数在定时中断中调用
 */
void Command_update(Command_t *command)
{
    // 上升指令
    if ((command->user_command > command->true_command) && (command->flag == 0))
    {
        command->flag = 1;
    }
    else if ((command->user_command > command->true_command) && (command->flag == 1))
    {
        command->flag = 1;
        command->true_command = command->true_command + command->k * command->sample_time;
    }
    else if ((command->user_command > command->true_command) && (command->flag == -1))
    {
        command->flag = 0;
        command->true_command = command->user_command;
    }
    ...
    if (command->user_command == command->true_command)
    {
        command->flag = 0;
    }
}
 ...
 * @param   command     指令句柄
 * @param   k           指令斜率
 * @param   init_ref    指令初始值
 * @param   t           采样时间
 */
void Command_init(Command_t *command, float k, float init_ref, float t)
{
    command->k = k;
    command->sample_time = t;
    command->user_command = init_ref;
    command->true_command = init_ref;
}
